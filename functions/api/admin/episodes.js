...
export async function createEpisode(request, env) {
  try {
    const { drama_id, episode_number, title, video_url, duration, is_free, price, status, language } = await request.json();
    const id = crypto.randomUUID();
    await env.MY_DB.prepare(
    'INSERT INTO episodes (id, drama_id, episode_number, title, video_url, duration, price, status, language) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
).bind(
    id, drama_id, episode_number, title, video_url, duration || 0,
    price !== undefined ? price : 0,  // 只用 price
    status || 'published', language || 'en-US'
).run();
...
export async function updateEpisode(request, env, id) {
    try {
        const { episode_number, title, video_url, duration, price, status, language } = await request.json();
        await env.MY_DB.prepare(
            'UPDATE episodes SET episode_number = ?, title = ?, video_url = ?, duration = ?, price = ?, status = ?, language = ? WHERE id = ?'
        ).bind(
            episode_number, title, video_url, duration || 0,
        ...
        for (const ep of episodes) {
            try {
                ...
                await env.MY_DB.prepare(
                    ...
                    id, drama_id, ep.episode_number, ep.title, ep.video_url, ep.duration || 0,
                    ep.price !== undefined ? ep.price : 0,  // 添加 price 字段
                    ep.status || 'published', ep.language || 'en-US'
                ).run();
